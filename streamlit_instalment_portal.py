...
with tabs[1]:
    ...
    # Input Fields
    # -------------------------
            net_salary = st.number_input("Monthly Net Profit (PKR)", min_value=0)
            applicant_bank_balance = st.number_input("Applicant’s Average 6-Month Bank Balance (PKR)", min_value=0)
            guarantor_bank_balance = st.number_input("Guarantor’s Average 6-Month Bank Balance (PKR)", min_value=0)
            job_tenure_years = st.number_input("Years in Business", min_value=0)
            dependents = st.number_input("Number of Dependents", min_value=0)
            age = st.number_input("Applicant Age", min_value=18, max_value=75)
            residence = st.selectbox("Residence Type", ["Owned", "Rented", "Mortgaged", "Other"])
            employer_type = st.selectbox("Business Type", ["Registered Company", "Sole Proprietorship", "Partnership", "Other"])
            outstanding = st.number_input("Outstanding Loan Amount (PKR)", min_value=0)
            emi = st.number_input("Monthly EMI Amount (PKR)", min_value=0)
            tenure = st.number_input("Loan Tenure (in months)", min_value=1)

            if st.button("Evaluate Business Applicant"):
        # -------------------------
        # Score Calculations
        # -------------------------
        ...
        st.write(f"**Employer Type (Business Type):** {employer_type_desc}")
